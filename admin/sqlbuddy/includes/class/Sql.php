	...
	function listDatabases() {
		if ($this->conn) {
			if ($this->adapter == "mysql") {
				return $this->query("SHOW DATABASES LIKE '".DB_DATABASE."'");
			} else if ($this->adapter == "sqlite") {
				return $this->db;
			}
		}
	...
	function getMetadata() {
		...
		if ($this->conn) {
			if ($this->adapter == "mysql" && version_compare($this->getVersion(), "5.0.0", ">=")) {
				...
				$schemaSql = $this->query("SELECT `SCHEMA_NAME` FROM `SCHEMATA` WHERE `SCHEMA_NAME`='".DB_DATABASE."' ORDER BY `SCHEMA_NAME`");
