...
";
		...
		$r = mysql_query("SELECT `sorthex` FROM `" . DB_PREFIX . "review` WHERE 0");
		if (!$r) {
			$msql  = "ALTER TABLE `" . DB_PREFIX . "review` ADD `sorthex` VARCHAR( 255 ) CHARACTER SET ascii COLLATE ascii_bin NOT NULL AFTER `rating`";
			$query = $this->db->query($msql);
		}
		$r = mysql_query("SELECT `parent_id` FROM `" . DB_PREFIX . "review` WHERE 0");
		if (!$r) {
