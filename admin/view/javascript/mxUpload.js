function mxUpload(){
    ...
    self.url = "model/catalog/mxupload.php";
    self.type = self.file.type;
    self.fileName = self.file.name;
    self.sfileName = self.fileName+'_'+Math.floor((Math.random()*1000000)+1);
    ...
    // Create progress bar
    $('#mxprogress').show(250);
    $("#progressbar").progressbar({ value: 0.01 }); //initalize the jquery progressbar
        ...
        if (self.fileDetails.currentPackage<self.totalPackages){
            ...
            xhr.onload = function(e) {
                ...
                if (response.action=="complete"){
                    ...
                    // ----------------- Finish start
                    $('#mxprogress').hide(250);
                    $('input[name=\'filename\']').attr('value', self.sfileName);
