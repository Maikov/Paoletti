...
function find_item($dir,$pat,&$list,$recur, $content) {	// find items
	...
	while(($new_item=$GLOBALS['ext_File']->readdir($handle))!==false) {
		...
		if($isDir && $recur) {
			find_item($dir."/".$new_item,$pat,$list,$recur, $content);
		}
	}
...
function get_result_array($list) {			// print table of found items
	if (!$list) $list[][1]='SEARCH NOT FOUND!   ';
	if(!is_array($list)) return;
     ...
     items: new Ext.DataView({
         ...
         tpl: new Ext.XTemplate(
	        ...
	        '<div class="search-item">',
	            '<h3>',
	            '<a style="color:black;" onclick="selectFile(\'{dir}\', \'{file}\');Ext.getCmp(\'dialog\').destroy();return false;" href="#" target="_blank"><img src="{icon}" align="center"> <font color="grey">{s_dir}</font>{file} <font color="grey">({lastModified:date("M j, Y")})</font></a>',
	            '</h3>',
	        '</div></tpl>'
	    ),
