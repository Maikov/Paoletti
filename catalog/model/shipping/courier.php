...
class ModelShippingcourier extends Model {
	function getQuote($address) {
		...
		if ($status) {
			...
			$rates = explode(',', $this->config->get('courier_rate'));
			...
			if ((float)$cost) {
				$quote_data['courier'] = array(
					...
					'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('courier_tax_class_id'), $this->config->get('config_tax')))
      			);
