...
class ControllerRecordRecord extends Controller
{
	...
	public function index()
	{
		...
		$product_file                = file_get_contents(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/product.tpl');
		$findme                      = 'colorbox';
		...
		if ($record_info) {
			...
			if ($record_info['image']) {
				$this->data['popup'] = '/image/' . $record_info['image'];
			}
			...
			if (!isset($record_info['date_available'])) $record_info['date_available']=$record_info['date_added'];
			...
			//print_r($this->data['blog_design']['blog_template_record']);
	...
	public function comment()
	{
		...
		// Ќе забываем про свои ссылки seo дл€ ajax запроса
		...
		//$pagination->url          = $this->url->link('record/record', 'record_id=' . $this->request->get['record_id'] . '&sorting=' . $this->data['sorting'] . '&page={page}&ajax=1');
