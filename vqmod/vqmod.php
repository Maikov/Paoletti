<?php
// vQMod with OpenShop folder and manual mode support
// ***********************************************************************************
...
final class VQMod {
	...
	public function __construct($path = false, $logging = true) {
		...
		if (!$this->manual) $this->_getMods();
        //MXE
		$this->_loadProtected();
	}

    // MXS--------------------------------------
    public function modCheck2($sourceFile) {
    ...
    public function RebuildCache() {
        ...
        $files=array();
        foreach ($list as $l)
            $files=array_merge($files,glob($l));
        $files=array_unique($files);
        sort($files);
        //foreach($files as $l) echo $l.'<br/>';
        // End build list
	...
	private function _getMods() {
        ...
        global $config;
        if (!file_exists($this->path('vqmod/xml/OpenShop/').'OpenShop_disabled')) 
          $this->_modFileList = array_merge($this->_modFileList , glob($this->path('vqmod/xml/OpenShop/') . '*.xml'));
